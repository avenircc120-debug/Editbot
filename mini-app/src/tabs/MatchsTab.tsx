import { useState, useEffect, useCallback } from 'react';
    ...
    // ── Carte unifiée : live, programmé, à venir, terminé ─────────────────────
    function MatchCard({ match, token, onToggle }: MatchCardProps) {
          ...
          <>
            {live.length > 0 && filter !== 'today' && (
              <>
                ...
                {live.map(m => <MatchCard key={m.match_id} match={m} token={token} onToggle={optimisticToggle} />)}
              </>
            )}
            ...
            {todayMtch.length > 0 && filter !== 'live' && (
              <>
                ...
                {todayMtch.map(m => <MatchCard key={m.match_id} match={m} token={token} onToggle={optimisticToggle} />)}
              </>
            )}
            ...
            {upcoming.length > 0 && filter !== 'live' && filter !== 'today' && (
              <>
                ...
                {upcoming.map(m => <MatchCard key={m.match_id} match={m} token={token} onToggle={optimisticToggle} />)}
              </>
            )}
            ...
            {finished.length > 0 && filter !== 'live' && (
              <>
                ...
                {finished.map(m => <MatchCard key={m.match_id} match={m} token={token} onToggle={optimisticToggle} />)}
              </>
            )}
