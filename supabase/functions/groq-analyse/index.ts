// groq-analyse/index.ts — COMPETITIONS (Sheets, API key) → ESPN → Groq
//   → Supabase DB + ANALYSE_IA_GROQ (Sheets, WIF)
...
// Colonnes onglet ANALYSE_IA_GROQ
// Timestamp | Competition | Match | Marché | Analyse_Groq | Prédiction | Confiance | Action | Envoyé
...
async function getESPNMatches(league: string): Promise<unknown[]> {
  try {
    ...
    if (!r.ok) return [];
    return ((await r.json()) as { events?: unknown[] }).events ?? [];
  } catch { return []; }
}
...
async function analyseWithGroq(
  ...
  const prompt =
    `Tu es un analyste football expert. Analyse ce match et donne une prédiction.\n\n` +
    `Compétition : ${competition}\nMatch       : ${matchDesc}\n` +
    `Marché      : ${marche}\nContexte ESPN: ${context}\n\n` +
    `Réponds UNIQUEMENT avec ce JSON (sans markdown) :\n` +
    `{"prediction":"<résultat>","confiance":<0-100>,"action":"<JOUER ou NE PAS JOUER>","detail":"<2-3 phrases>"}`;
  ...
  try {
    ...
    const json = JSON.parse(text.replace(/^```(?:json)?|```$/gm, "").trim()) as AnalyseResult & { confiance: number };
    return {
...
Deno.serve(async (req) => {
  ...
  // Lire COMPETITIONS via API key (lecture seule)
  // Colonnes : A=ID | B=Nom | C=ESPN_League_ID | D=Actif | E=Priorité | F=Marché_Defaut
  ...
  for (const comp of actives) {
    ...
    for (const ev of (events as Record<string, unknown>[]).slice(0, 5)) {
      ...
      const comps = (ev["competitions"] as {
        competitors?: { team?: { abbreviation?: string }; score?: string }[];
      ...
      // Écriture Google Sheets via WIF (ANALYSE_IA_GROQ tab)
      try {
        await sheetsAppend("ANALYSE_IA_GROQ!A:I", [[
          now,
          compNom,
          matchDesc,
          marcheCible,
          analyse.detail,
          analyse.prediction,
          String(analyse.confiance),
          analyse.action,
          "FALSE",
        ]], SHEETS_ID);
      } catch (e) {
        console.warn("Sheets append échoué (WIF non configuré ?):", String(e));
      }
